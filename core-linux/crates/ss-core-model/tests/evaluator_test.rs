...
fn evaluator_allows_when_no_rule_matches() {
    let pol = mk_policy(vec![Rule::AppBlocklist {
        matchers: vec![AppMatcher { content_hash: None, basename: Some("steam".into()), path_glob: None }],
    }]);
...
fn evaluator_blocks_on_blocklist_match() {
    ...
    assert_eq!(evaluate(&[pol], &p, 0, &Utc::now()), Action::Block { reason: "blocklist".into() });
}
...
fn evaluator_blocks_when_budget_exceeded() {
    ...
    assert_eq!(
        ...
        Action::Block { reason: "budget_exceeded".into() }
    );
...
fn evaluator_warns_near_budget() {
    ...
    assert_eq!(
        ...
        Action::Warn { reason: "budget_near".into(), remaining_minutes: 5 }
    );
}
...
fn evaluator_blocks_outside_time_window() {
    ...
    let p = proc_at("sha256:abc", "firefox", "/usr/bin/firefox");
    let monday_10h: DateTime<Utc> = NaiveDate::from_ymd_opt(2026, 4, 20).unwrap()
        .and_hms_opt(10, 0, 0).unwrap().and_utc();
    assert_eq!(evaluate(&[pol], &p, 0, &monday_10h), Action::Block { reason: "window_closed".into() });
}

#[test]
fn evaluator_allowlist_blocks_everything_else() {
    let pol = mk_policy(vec![Rule::AppAllowlist {
        matchers: vec![AppMatcher { content_hash: None, basename: Some("firefox".into()), path_glob: None }],
    }]);
    let steam = proc_at("sha256:abc", "steam", "/usr/bin/steam");
    let firefox = proc_at("sha256:def", "firefox", "/usr/bin/firefox");
    assert_eq!(evaluate(std::slice::from_ref(&pol), &steam, 0, &Utc::now()), Action::Block { reason: "not_in_allowlist".into() });
...
fn evaluator_blocklist_wins_over_allowlist() {
    ...
    let pol = mk_policy(vec![
        Rule::AppAllowlist { matchers: vec![AppMatcher { content_hash: None, basename: Some("firefox".into()), path_glob: None }] },
        Rule::AppBlocklist { matchers: vec![AppMatcher { content_hash: None, basename: Some("firefox".into()), path_glob: None }] },
    ]);
    let firefox = proc_at("sha256:def", "firefox", "/usr/bin/firefox");
    assert_eq!(evaluate(&[pol], &firefox, 0, &Utc::now()), Action::Block { reason: "blocklist".into() });
}
