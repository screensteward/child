...
fn parent_and_child_crud() {
    ...
    ss_core_store::dao::family::insert(&s, &Family {
        id: fam_id, name: "F".into(),
        created_at: Utc::now(), modified_at: Utc::now(),
    }).unwrap();
    let p = Parent {
        id: Uuid::new_v4(), family_id: fam_id,
    ...
    let c = Child {
        id: Uuid::new_v4(), family_id: fam_id,
        display_name: "Bob".into(), birth_year: Some(2015),
        created_at: Utc::now(), modified_at: Utc::now(),
    };
...
fn device_crud() {
    ...
    ss_core_store::dao::child::insert(&s, &Child {
        id: child_id, family_id: fam_id,
        display_name: "Bob".into(), birth_year: None,
        created_at: Utc::now(), modified_at: Utc::now(),
    }).unwrap();
    let d = ChildDevice {
        id: Uuid::new_v4(), child_id,
        hostname: "workstation".into(), platform: Platform::Linux,
